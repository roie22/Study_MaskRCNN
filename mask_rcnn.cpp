// Copyright (C) 2018-2019, BigVision LLC (LearnOpenCV.com), All Rights
// Reserved. Author : Sunita Nayak Article :
...
int main_exc(int argc, char **argv) {
    ...
    String classesFile = "../deps/Model/mscoco_labels.names";
    ...
    // Load the colors
    String colorsFile = "../deps/Model/colors.txt";
    ...
    String textGraph =
        "../deps/Model/mask_rcnn_inception_v2_coco_2018_01_28.pbtxt";
    textGraph = parser.get<String>("config");
    std::cout << "config = " << textGraph << std::endl;

    String modelWeights = "../deps/Model/mask_rcnn_inception_v2_coco_2018_01_28/"
    ...
    std::string str;
    try {
        // outputFile = "../deps/TestData/mask_rcnn_out_cpp.avi";
    ...
    while (waitKey(1) < 0) {
        ...
        blobFromImage(frame, blob, 1.0, Size(frame.cols, frame.rows), Scalar(),
                      true, false);
        ...
        string label = format("Mask-RCNN on 2.5 GHz Intel Core i7 CPU, Inference "
                              "time for a frame : %0.0f ms",
                              t);
...
int main() {
    ...
    char *argv[] = {
                    "appMaskRcnn",
                    "-v=../deps/TestData/cars.mp4",
                    "--classes=../deps/Model/mask_rcnn/mscoco_labels.names",
                    "--config=../deps/Model/mask_rcnn/"
                    "mask_rcnn_inception_v2_coco_2018_01_28.pbtxt",
                    "--weight=../deps/Model/mask_rcnn/mask_rcnn_inception_v2_coco_2018_01_28/"
                    "frozen_inference_graph.pb",
                    "--color=../deps/Model/mask_rcnn/colors.txt",
                    "--output=./out.avi"};
...
void drawBox(Mat &frame, int classId, float conf, Rect box, Mat &objectMask) {
    ...
    rectangle(frame, Point(box.x, box.y),
              Point(box.x + box.width, box.y + box.height), Scalar(255, 178, 50),
              3);
    ...
    putText(frame, label, Point(box.x, box.y), FONT_HERSHEY_SIMPLEX, 0.75,
            Scalar(0, 0, 0), 1);
